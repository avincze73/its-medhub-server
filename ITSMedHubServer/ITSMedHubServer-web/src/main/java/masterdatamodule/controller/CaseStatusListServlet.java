 ...
 * @author tds
 */
@WebServlet(name = "CaseStatusListServlet", urlPatterns = {"/pages/CaseStatusListServlet"})
public class CaseStatusListServlet extends MasterDataServletBase<CaseStatus> {

    @Inject
    private CaseStatusController controller;
    
   
    @Override
    protected MasterDataControllerBase<CaseStatus> getController() {
        return controller;
    }

    @Override
    protected void putData(JSONArray rowData, CaseStatus entity) {
    ...
    protected void doSort(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (request.getParameter("sidx").equals("englishName")) {
            Collections.sort(controller.getEntityList(), new Comparator<CaseStatus>() {
