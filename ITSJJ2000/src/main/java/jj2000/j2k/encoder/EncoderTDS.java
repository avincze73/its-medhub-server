 ...
 *
 * A softvare jelenlegi modositott valtozataban az image data-t nem file-bol
 * hanem input stream-bol varja, es a kodolt adatokat file helyett
 * output stream-be helyezi. A megadhato opciok korlatozva vannak.
 * */
...
public class EncoderTDS
{
	...
	private OutDataTDS outData;
	private Float compressRatio;


	/**
	 * EncoderTDS objektum letrehozasa.
	 * @param data image data
	 * @param bitDepth
	 * @param compressRatio tomorites merteke. Ha null, akkor lossless. Ha 0 - 1 kozott van, akkor azt hatarozza meg, hogy az eredeti
	 * meret hanyszorosara tomoritse.
	 */
	public EncoderTDS(int[] data, int bitDepth, int width, int height, boolean isSigned, Float compressRatio)
	{
		this.data = data;
		...
		ParameterList defParamList = new ParameterList();
		String[][] param = getAllParameters();		// az osszes lehetseges parameter lekerdezese, parameterenkent 4 adattal

		// a default ertekkel rendelkezo parameterek nevenek es default ertekenek betoltese a defParamList-be.
        for (int i = param.length-1; i >= 0; i--)
		{
			if(param[i][3] != null)
				defParamList.put(param[i][0],param[i][3]);
		}

		// Create parameter list using defaults
        ParameterList paramList = new ParameterList(defParamList);

		// argumentumok letrehozasa az eljaras parameterei alapjan
		String[] arguments = new String[2];
		if(compressRatio == null)
		{
			arguments[0] = "-lossless";
			arguments[1] = "on";
		}
		else	// lossy
		{
			float rate = (float)bitDepth * compressRatio.floatValue();
			arguments[0] = "-rate";
			arguments[1] = Float.toString(rate);
		}

		// Parse arguments
		paramList.parseArgs(arguments);
     ...
     * values);
     * */
    public EncoderTDS(ParameterList pl) {
	this.pl = pl;
	defpl = pl.getDefaultParameterList();
    }
     ...
     * @see #getExitCode
     * */
    public void start() throws J2KTDSException, IOException
	{
        boolean verbose;
...
//            imreadervec = null;
			...
			// Image reader beallitas
			imreader = new ImgReader[1];
			imreader[0] = new ImgReaderTDS(data, bitDepth, width, height, isSigned);
			ncomp = 1;
			ppminput = false;
...
//                return;
			...
			{
				throw new J2KTDSException("WARNING! Input image is RGB and no color transform has "+
...
//                return;
            ...
            // **** CodestreamWriter ****
			int expectedSize = 0;
			if(compressRatio != null) expectedSize = (int) (1.1 * (float) (width * height * bitDepth) * compressRatio.floatValue() / 8);
			else expectedSize = (int) ((float) (width * height * bitDepth) * 0.8 / 8);
            ...
            try {
                ...
                bwriter = new FileCodestreamWriter(baos,Integer.MAX_VALUE);
            } catch (IOException e) {
...
//            }
			...
			outData = new OutDataTDS();
			outData.data = baos.toByteArray();
			outData.endPtr = outData.data.length;

            // **** Tile-parts and packed packet headers ****
            if(pktspertp>0 || pphTile || pphMain) {
                int headInc;
                try {
                    CodestreamManipulatorTDS cm = new
                        CodestreamManipulatorTDS(outData.data, expectedSize, ntiles, pktspertp,
                                              pphMain,pphTile,tempSop,tempEph);
                    fileLength += cm.doCodestreamManipulation();
					outData = cm.getOutData();
                    String res="";
//                    if(pktspertp>0) {
//                        FacilityManager.
...
//        }
     ...
     * */
    private void error(String msg, int code) throws J2KTDSException
	{
        exitCode = code;
//        FacilityManager.getMsgLogger().printmsg(MsgLogger.ERROR,msg);
		throw new J2KTDSException(msg);
    }
