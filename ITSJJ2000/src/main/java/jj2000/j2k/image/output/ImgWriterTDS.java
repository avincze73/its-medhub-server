 ...
 * @see BlkImgDataSrc
 * */
public class ImgWriterTDS extends ImgWriter
{
    ...
    private RandomAccessByteArrayTDS out;
     ...
     * @see DataBlk
     * */
    public ImgWriterTDS(BlkImgDataSrc imgSrc, int c, boolean isSigned) throws IOException
	{
        //Initialize
        this.c = c;
...
//        }
		...
		this.out = new RandomAccessByteArrayTDS(null, w * h * packBytes);
...
//    }
    ...
    public void close() throws IOException
	{
        ...
        if(out.length() != w*h*packBytes+offset)
		{
			try
			{
				...
				out.seek(out.length());
				// Fill with 0s
				byte[] b = new byte[offset+w*h*packBytes-(int)out.length()];
	...
	//            }
		...
		oimg = new OutputImgTDS();
		oimg.data = out.getOutData().data;
		oimg.endPtr = out.getOutData().endPtr;
		oimg.bitDepth = bitDepth;
		oimg.width = w;
		oimg.height = h;
