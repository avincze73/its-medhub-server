 ...
 * */
public class ImgReaderTDS extends ImgReader implements EndianType{

    /** The offset of the raw pixel data in the PGX file */
    private int offset;

    /** The RandomAccessIO where to get datas from */
    private RandomAccessArrayInTDS in;
     ...
     * */
    public ImgReaderTDS(int[] data, int bitDepth, int width, int height, boolean isSigned) throws IOException, J2KTDSException
	{
        if(data == null)
			throw new IllegalArgumentException("Image data is null.");

        //Opens the given file... HELYETT: letrehozza a RandomAccessFile-t imitalo RandomAccessArray-t!
        this.in = new RandomAccessArrayInTDS(data, bitDepth);
//        try{
