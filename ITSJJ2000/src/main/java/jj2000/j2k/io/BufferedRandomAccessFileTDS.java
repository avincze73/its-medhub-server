...
import jj2000.j2k.util.RandomAccessByteArrayTDS;
 ...
 * */
public abstract class BufferedRandomAccessFileTDS implements RandomAccessIO, EndianType
{
    ...
    /**
     * The RandomAccessFile associated with the buffer, HELYETT: egy azt imitalo RandomAccessByteArrayTDS osztaly
     * */
    private RandomAccessByteArrayTDS theFile;
     ...
     * */
    protected BufferedRandomAccessFileTDS(byte[] outData, int expectedSize, String mode, int bufferSize) throws IOException, J2KTDSException
	{
		this.outData = outData;
		
		fileName = "non file version (TDS)";
		...
		theFile = new RandomAccessByteArrayTDS(this.outData, expectedSize);
...
//    }
     ...
     * @exception java.io.IOException If an I/O error ocurred.
     * */
    protected BufferedRandomAccessFileTDS(byte[] outData, int expectedSize, String mode) throws IOException, J2KTDSException
	{
		this(outData, expectedSize, mode, 512);
    }

	public OutDataTDS getOutData()
	{
		return theFile.getOutData();
	}
     ...
     * @exception java.io.IOException If an I/O error ocurred.
     * */
    protected final void readNewBuffer(int off) throws IOException, J2KTDSException{
     ...
     * @exception java.io.IOException If an I/O error ocurred.
     * */
    public final void write(byte b) throws IOException, J2KTDSException{
	// As long as pos is less than the length of the buffer we can write
	// to the buffer. If the position is after the buffer a new buffer is
	// needed
	if(pos<byteBuffer.length){
    ...
    public final void write(byte[] b, int offset, int length)
        throws IOException, J2KTDSException{
        int i,stop;
        stop = offset+length;
        if(stop > b.length)
