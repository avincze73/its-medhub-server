...
//    }
     ...
     * @see #getExitCode
     * */
    public void start() throws IOException, ICCProfileException, J2KTDSException
	{
...
//            if(infile.indexOf("/") >= 1 &&
//					infile.charAt(infile.indexOf("/")-1) == ':')
//			{ // an URL
//                URL inurl;
//                URLConnection conn;
//                int datalen;
...
//	    if(disp)
//		{ // No output file has been specified. Display
//		// decoded image
//
...
//			{
			...
			try
			{
				if(csMap!=null)
				{
					imwriter[i] = new ImgWriterTDS(decodedImage, i, csMap.isOutputSigned(i));
				}
				else
				{
					imwriter[i] = new ImgWriterTDS(decodedImage, i, hd.isOriginalSigned(i));
				}
			}
			catch (IOException e) {
...
//                        }
			...
			{
				imwriter[i].close();
				outImg = ((ImgWriterTDS) imwriter[i]).getOutputImg();
		    }
			catch (IOException e) {
...
//        }
	...
	public OutputImgTDS getOutputImg()
	{
		return outImg;
	}
...
//        FacilityManager.getMsgLogger().printmsg(MsgLogger.ERROR,msg);
     ...
     * */
    private void error(String msg, int code, Throwable ex) throws J2KTDSException
	{
		exitCode = code;
//		FacilityManager.getMsgLogger().printmsg(MsgLogger.ERROR,msg);
//		if(pl.getParameter("debug").equals("on")) {
...
//			error("Use '-debug' option for more details",2);
//		}
		throw new J2KTDSException(msg, ex);
    }
